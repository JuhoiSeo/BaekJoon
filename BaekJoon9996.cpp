/*
	...
	http://boj.kr/634a23121b1942c58cc98d7f2f8750fe
*/
...
int main(){
	...
	for(int i = 0; i < num; i++){
		...
		for(int j = 0; j < ind; j++){
			if(pat[j] != str[j]) goto a;
		}
		
		for(int j = 0; j < (pat.length() - ind - 1); j++){
			if(pat[pat.length() - j - 1] != str[str.length() - j - 1]) goto a;
		}
	}
...
/*
	...
	패턴을 알파벳 소문자 여러 개와 별표(*) 하나로 이루어진 문자열이다.
